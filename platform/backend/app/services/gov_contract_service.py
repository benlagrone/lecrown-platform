...
DART_BONFIRE_PORTAL_URL = "https://dart.bonfirehub.com/portal"
DART_BONFIRE_OPEN_OPPORTUNITIES_URL = "https://dart.bonfirehub.com/PublicPortal/getOpenPublicOpportunitiesSectionData"
DART_OPEN_CONTEXT = "dart_open_procurement"
SOURCE_LABELS = {
...
TRACKED_PROCUREMENT_SOURCE_DEFINITIONS: tuple[GovContractTrackedSourceDefinition, ...] = (
    ...
    GovContractTrackedSourceDefinition(
        ...
        extraction_mode="json_api",
        load_scope="opportunities",
        automation_summary="Weekly Bonfire JSON import",
        automation_detail="Loads the public Bonfire open-opportunities JSON and stores scored opportunities.",
        notes="Only public opportunities with valid project identifiers are imported.",
    ),
    GovContractTrackedSourceDefinition(
...
DART_SOURCE_CONTEXT_LABELS = {DART_OPEN_CONTEXT: "Open procurement"}
...
def _request_json_payload(
    ...
    *,
    ...
    if not isinstance(payload, dict):
        raise GovContractSourceError(f"{source_label} returned a non-object JSON payload")
    return payload
...
def _parse_bonfire_datetime(value: str | None) -> datetime | None:
    if not value:
        return None
...
def _bonfire_due_parts(value: str | None) -> tuple[date | None, str | None]:
    parsed = _parse_bonfire_datetime(value)
    if parsed is None:
        return None, None
    return parsed.date(), parsed.strftime("%I:%M %p").lstrip("0")
...
def _dart_project_url(project: dict[str, object]) -> str:
    project_id = _clean(project.get("ProjectID"))
    private_project_id = _clean(project.get("PrivateProjectID"))
    visibility_id = _clean(project.get("ProjectVisibilityID"))
    if visibility_id and visibility_id != "1" and private_project_id:
        return f"https://dart.bonfirehub.com/opportunities/private/{private_project_id}"
    return f"https://dart.bonfirehub.com/opportunities/{project_id}"


def _build_dart_record(
    *,
    definition: GovContractTrackedSourceDefinition,
    project: dict[str, object],
) -> GovContractSourceRecord:
    project_id = _clean(project.get("ProjectID")) or "unknown-project"
    solicitation_id = _clean(project.get("ReferenceID")) or project_id
    title = _clean(project.get("ProjectName")) or solicitation_id
    due_date, due_time = _bonfire_due_parts(_clean(project.get("DateClose")))
    return GovContractSourceRecord(
        source_key=f"{DART_PROCUREMENT_SOURCE_NAME}:{DART_OPEN_CONTEXT}:{project_id}",
        solicitation_id=solicitation_id,
        source_url=_dart_project_url(project),
        title=title,
        agency_name=definition.label,
        agency_number=_clean(project.get("DepartmentID")),
        status_code="1",
        status_name="Open Opportunity",
        due_date=due_date,
        due_time=due_time,
        posting_date=None,
        source_created_at=None,
        ...
        raw_payload={
            ...
            "private_project_id": project.get("PrivateProjectID"),
            "project_status_id": project.get("ProjectStatusID"),
            "project_sub_status_id": project.get("ProjectSubStatusID"),
            "project_visibility_id": project.get("ProjectVisibilityID"),
            "department_id": project.get("DepartmentID"),
            "date_close": project.get("DateClose"),
            "portal": "Bonfire",
            "source_context": DART_OPEN_CONTEXT,
            "source_context_label": DART_SOURCE_CONTEXT_LABELS[DART_OPEN_CONTEXT],
        },
    )


def fetch_dart_contracts() -> GovContractFetchResult:
    definition = TRACKED_PROCUREMENT_SOURCES[DART_PROCUREMENT_SOURCE_NAME]
    payload = _request_json_payload(DART_BONFIRE_OPEN_OPPORTUNITIES_URL, source_label=definition.label)
    if payload.get("success") != 1 or not isinstance(payload.get("payload"), dict):
        raise GovContractSourceError("DART returned an unsuccessful or malformed response")
    projects = payload["payload"].get("projects")
    if isinstance(projects, dict):
        projects = list(projects.values())
    if not isinstance(projects, list):
        raise GovContractSourceError("DART response is missing its projects collection")
    records = []
    seen = set()
    for project in projects:
        if not isinstance(project, dict):
            raise GovContractSourceError("DART returned a malformed project")
        project_id = _clean(project.get("ProjectID"))
        if not project_id:
            raise GovContractSourceError("DART project is missing ProjectID")
        if _clean(project.get("ProjectVisibilityID")) != "1" or project_id in seen:
            continue
        seen.add(project_id)
        records.append(_build_dart_record(definition=definition, project=project))
    return GovContractFetchResult(
        request_payload={"url": DART_BONFIRE_OPEN_OPPORTUNITIES_URL},
        source_total_records=len(projects),
        csv_text=_records_to_simple_csv(records),
        records=records,
    )
...
def refresh_dart_contracts(db: Session) -> GovContractImportRun:
    run = _create_single_day_import_run(db, DART_PROCUREMENT_SOURCE_NAME)
    try:
        fetched = fetch_dart_contracts()
        return _persist_source_records(
            db,
            run=run,
            source_name=DART_PROCUREMENT_SOURCE_NAME,
            fetched=fetched,
            is_open_resolver=lambda record, *, today: _is_open_contract(record.status_code, record.due_date, today=today),
        )
    except GovContractSourceError as exc:
...
def refresh_tracked_procurement_sources(db: Session) -> list[GovContractImportRun]:
    ...
    for definition in TRACKED_PROCUREMENT_SOURCE_DEFINITIONS:
        try:
            ...
            elif definition.source == DART_PROCUREMENT_SOURCE_NAME:
                runs.append(refresh_dart_contracts(db))
            else:
